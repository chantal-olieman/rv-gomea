...
double installedProblemLowerRangeBound(int index, int dimension) {
    switch (index) {
        ...
        case 13:
            return (sumOfEllipsoidsFunctionLowerRangeBound(dimension));
    }
...
double installedProblemUpperRangeBound(int index, int dimension) {
    switch (index) {
        ...
        case 13:
            return (sumOfEllipsoidsFunctionUpperRangeBound(dimension));
    }
...
void installedProblemEvaluation(int index, double *parameters, double *objective_value, double *constraint_value) {
    ...
    if (rotation_angle == 0.0)
        ...
        for (i = 0; i < number_of_ellipsoids; i++) {
            ...
            rotated_parameters_ellipsoid = matrixVectorMultiplication(rotation_matrix, parameters_ellipsoid,
                                                                      ellipsoid_size, ellipsoid_size);
            for (j = 0; j < ellipsoid_size; j++)
    ...
    *constraint_value = 0.0;
    ...
    switch (index) {
        ...
        case 12:
            ...
            break;
        case 13:
            sumOfEllipsoidsFunctionProblemEvaluation(parameters, objective_value, constraint_value);
            break;
    }
}
...
void initializePopulationAndFitnessValues(int population_index) {
    ...
    for (j = 0; j < population_sizes[population_index]; j++) {
        ...
        } else if (betterFitness(objective_values[population_index][j], constraint_values[population_index][j],
                                 population_best_obj_val[population_index],
                                 population_best_constraint_val[population_index])) {
            population_best_obj_val[population_index] = objective_values[population_index][j];
            population_best_constraint_val[population_index] = constraint_values[population_index][j];
        }
    }
...
void initializeObjectiveRotationMatrix(void) {
    ...
    for (index0 = 0; index0 < ellipsoid_size - 1; index0++) {
        for (index1 = index0 + 1; index1 < ellipsoid_size; index1++) {
            ...
            product = matrixMatrixMultiplication(matrix, rotation_matrix, ellipsoid_size, ellipsoid_size,
                                                 ellipsoid_size);
            for (i = 0; i < ellipsoid_size; i++)
                for (j = 0; j < ellipsoid_size; j++)
...
void writeGenerationalStatisticsForOnePopulation(int population_index) {
    ...
    for (j = 0; j < number_of_populations; j++) {
        if (betterFitness(population_best_obj_val[j], population_best_constraint_val[j], overall_best_obj,
                          overall_best_cons)) {
    ...
    sprintf(string, "%10d %10d %11.3lf %15.10e %13e  ", total_number_of_generations, number_of_evaluations, getTimer(),
            overall_best_obj, overall_best_cons);
    fputs(string, file);

    sprintf(string, "[ %4d %6d %10d %13e %13e %13e %13e %13e %13e %13e %13e %13e ]\n", population_index,
            number_of_generations[population_index], population_sizes[population_index],
            distribution_multipliers[population_index], population_objective_best, population_objective_avg,
            population_objective_var, population_objective_worst, population_constraint_best, population_constraint_avg,
            population_constraint_var, population_constraint_worst);
    fputs(string, file);
...
void makeSelectionsForOnePopulationUsingDiversityOnRank0(int population_index) {
    ...
    for (i = 0; i < number_of_rank0_solutions; i++)
        nn_distances[i] = distanceInParameterSpace(populations[population_index][preselection_indices[i]],
                                                   populations[population_index][selection_indices[
                                                           number_selected_so_far - 1]]);
...
double getSimilarity(int a, int b) {
    if (pruning_ub < number_of_parameters && mpm_number_of_indices[a] + mpm_number_of_indices[b] > pruning_ub)
        return (0);
    return (S_matrix[a][b]);
}
...
int determineNearestNeighbour(int index, double **S_matrix, int *mpm_number_of_indices, int mpm_length) {
    ...
    if (result == index)
        result++;
    for (i = 1; i < mpm_length; i++) {
        if (((S_matrix[index][i] > S_matrix[index][result]) || ((S_matrix[index][i] == S_matrix[index][result]) &&
                                                                (mpm_number_of_indices[i] <
                                                                 mpm_number_of_indices[result]))) && (i != index))
            result = i;
    }
    ...
    for (i = 0; i < linkage_model[population_index]->length; i++) {
        for (j = 0; j < linkage_model[population_index]->set_length[i]; j++) {
            for (k = j; k < linkage_model[population_index]->set_length[i]; k++) {
                ...
                cov = 0.0;
                for (m = 0; m < selection_sizes[population_index]; m++)
                    cov += (selections[population_index][m][a] - mean_vectors[population_index][a]) *
                           (selections[population_index][m][b] - mean_vectors[population_index][b]);
...
void generateAndEvaluateNewSolutionsToFillPopulation(int population_index) {
    ...
    alpha_AMS = 0.5 * tau *
                (((double) population_sizes[population_index]) / ((double) (population_sizes[population_index] - 1)));
    delta_AMS = 2;
        ...
        for (j = 1; j < population_sizes[population_index]; j++) {
            ...
            if ((number_of_generations[population_index] > 0) && (q < number_of_AMS_solutions)) {
                ...
                while ((out_of_range == 1) && (shrink_factor > 1e-10)) {
                    ...
                    out_of_range = 0;
                    for (k = 0; k < number_of_parameters; k++) {
                        solution_AMS[k] = solution[k] +
                                          shrink_factor * delta_AMS * distribution_multipliers[population_index] *
                                          (mean_vectors[population_index][k] -
                                           mean_vectors_previous[population_index][k]);
...
void computeParametersForSampling(int population_index) {
    ...
    if (!use_univariate_FOS) {
        cholesky_factors_lower_triangle[population_index] = (double ***) Malloc(
                linkage_model[population_index]->length * sizeof(double **));
        for (i = 0; i < linkage_model[population_index]->length; i++) {
            cholesky_factors_lower_triangle[population_index][i] = choleskyDecomposition(
                    covariance_matrices[population_index][i], linkage_model[population_index]->set_length[i]);
        }
    }
}
            ...
            for (i = 0; i < linkage_model[population_index]->length; i++) {
                ...
                if (use_univariate_FOS) {
                    ...
                    FOS_result = matrixVectorMultiplication(cholesky_factors_lower_triangle[population_index][i], z,
                                                            ...
                                                            linkage_model[population_index]->set_length[i]);
                    for (j = 0; j < linkage_model[population_index]->set_length[i]; j++)
                        result[linkage_model[population_index]->sets[i][j]] = FOS_result[j] +
...
double getStDevRatio(int population_index, double *parameters) {
    ...
    if (use_univariate_FOS) {
        ...
        for (i = 0; i < linkage_model[population_index]->length; i++) {
            inverse = matrixLowerTriangularInverse(cholesky_factors_lower_triangle[population_index][i],
                                                   linkage_model[population_index]->set_length[i]);

            x_min_mu = (double *) Malloc(linkage_model[population_index]->set_length[i] * sizeof(double));

            for (j = 0; j < linkage_model[population_index]->set_length[i]; j++)
                x_min_mu[j] = parameters[linkage_model[population_index]->sets[i][j]] -
                              mean_vectors[population_index][linkage_model[population_index]->sets[i][j]];

            z = matrixVectorMultiplication(inverse, x_min_mu, linkage_model[population_index]->set_length[i],
                                           linkage_model[population_index]->set_length[i]);
