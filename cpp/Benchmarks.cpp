...
double **Benchmarks::readOvectorVec() {
    ...
    int *temp_s = (int *) malloc(s_size * sizeof(int));
    ...
    if (file.is_open()) {
        ...
        while (getline(file, line)) {
            ...
            while (getline(iss, value, ',')) {
                ...
                int s_val = s[i];
                if((c-(up-temp_s[i]))<s_val){
                    d[i][c - (up - temp_s[i])] = stod(value);
                }
                // printf("1\n");
                c++;
            }
...
double Benchmarks::rastrigin(double *x, int dim, int k) {
    ...
    for (i = dim / k - 1; i >= 0; i--) {
        result +=
                x[Pvector[dim / k + i]] * x[Pvector[dim / k + i]] - 10.0 * cos(2 * PI * x[Pvector[dim / k + i]]) + 10.0;
    }
    return (result);
}
